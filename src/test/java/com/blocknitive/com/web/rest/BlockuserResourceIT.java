...
import com.blocknitive.com.service.criteria.BlockuserCriteria;
...
class BlockuserResourceIT {
     ...
     *
     * @see com.blocknitive.com.repository.search.BlockuserSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    void getAllBlockusersByBlockeduserIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, Appuser.class).isEmpty()) {
            blockeduser = AppuserResourceIT.createEntity(em);
            em.persist(blockeduser);
            em.flush();
        } else {
            blockeduser = TestUtil.findAll(em, Appuser.class).get(0);
        }
        em.persist(blockeduser);
        em.flush();
    ...
    void getAllBlockusersByBlockinguserIsEqualToSomething() throws Exception {
        // Initialize the database
        blockuserRepository.saveAndFlush(blockuser);
        Appuser blockinguser;
        if (TestUtil.findAll(em, Appuser.class).isEmpty()) {
            blockinguser = AppuserResourceIT.createEntity(em);
            em.persist(blockinguser);
            em.flush();
        } else {
            blockinguser = TestUtil.findAll(em, Appuser.class).get(0);
        }
        em.persist(blockinguser);
        em.flush();
    ...
    void getAllBlockusersByCblockeduserIsEqualToSomething() throws Exception {
        // Initialize the database
        blockuserRepository.saveAndFlush(blockuser);
        Community cblockeduser;
        if (TestUtil.findAll(em, Community.class).isEmpty()) {
            cblockeduser = CommunityResourceIT.createEntity(em);
            em.persist(cblockeduser);
            em.flush();
        } else {
            cblockeduser = TestUtil.findAll(em, Community.class).get(0);
        }
        em.persist(cblockeduser);
        em.flush();
    ...
    void getAllBlockusersByCblockinguserIsEqualToSomething() throws Exception {
        // Initialize the database
        blockuserRepository.saveAndFlush(blockuser);
        Community cblockinguser;
        if (TestUtil.findAll(em, Community.class).isEmpty()) {
            cblockinguser = CommunityResourceIT.createEntity(em);
            em.persist(cblockinguser);
            em.flush();
        } else {
            cblockinguser = TestUtil.findAll(em, Community.class).get(0);
        }
        em.persist(cblockinguser);
        em.flush();
    ...
    void searchBlockuser() throws Exception {
        ...
        // Initialize the database
        blockuserRepository.saveAndFlush(blockuser);
        when(mockBlockuserSearchRepository.search("id:" + blockuser.getId(), PageRequest.of(0, 20)))
