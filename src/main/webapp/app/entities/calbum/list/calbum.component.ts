...
export class CalbumComponent implements OnInit {
  ...
  search(query: string): void {
    if (query && ['title'].includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
  ...
  trackId(index: number, item: ICalbum): number {
    return item.id!;
  }

  delete(calbum: ICalbum): void {
    const modalRef = this.modalService.open(CalbumDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.calbum = calbum;
  ...
  protected onSuccess(data: ICalbum[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
